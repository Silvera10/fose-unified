...
/* Saldo final de un trimestre
   Saldo = Presup. Disponible - Compromisos - Gastos manuales (sin contrato)
   Los pagos de contratos están cubiertos por su compromiso, no se restan doble */
function getSaldoFinal(d, cod, trim){
  const pd = getPresupDisp(d, cod, trim);
  const comp = getCompromisoEgTrim(d, cod, trim);
  // Gastos manuales (no vinculados a contrato) se restan aparte
  const gastosManuales = (d.contratos||[])
    .filter(e => e.cod_rubro === cod && Number(e.trim) === trim && !e.contrato_full_id)
    .reduce((s,e) => s + (Number(e.valor)||0), 0);
  return pd - comp - gastosManuales;
}

/* CDP (Disponibilidad) registrada para un rubro/trimestre */
function getCdpEgTrim(d, cod, trim){
  return (d.cdps_eg||[])
    .filter(e => e.cod === cod && Number(e.trim) === trim)
    .reduce((s,e) => s + (Number(e.valor)||0), 0);
}

/* CDP de un rubro en un mes específico */
function getCdpMes(d, cod, mes){
  return (d.cdps_eg||[])
    .filter(e => e.cod === cod && Number(e.mes) === mes)
    .reduce((s,e) => s + (Number(e.valor)||0), 0);
}

/* CDP acumulados hasta un trimestre (inclusive) */
function getCdpHasta(d, cod, trim){
  let total = 0;
  for(let t=1; t<=trim; t++) total += getCdpEgTrim(d, cod, t);
  return total;
}
