...
function _snapshotCounts(d){
  return {
    ...
    trimHash: _hashObj(d.contratos||[])
  };
}

function _hashObj(obj){
  // Hash simple pero efectivo: usa suma de char codes del JSON
  try {
...
function duplicarVigencia(nuevaVigencia){
  ...
  DB.addInst(nuevoNombre).then(async (id) => {
    ...
    // Copiar mapeo SIFSE (nombres correctos: sifse_map_eg y sifse_map_ing)
    if(d.sifse_map_eg) newD.sifse_map_eg = JSON.parse(JSON.stringify(d.sifse_map_eg));
    if(d.sifse_map_ing) newD.sifse_map_ing = JSON.parse(JSON.stringify(d.sifse_map_ing));
