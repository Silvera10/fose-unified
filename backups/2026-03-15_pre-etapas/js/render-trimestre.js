...
R.trimestre = function(t){
  ...
  let totM=[0,0,0],totGasAcum=0,totComp=0,totCompAcum=0,totSaldo=0;
  ...
  d.rubros.forEach(r => {
    ...
    if (esGrupo){
      ...
      const gm = meses.map(mes => sumarHojasEg(d, r.cod, cod => getGastosMes(d, cod, mes)));
      const gasAcum = sumarHojasEg(d, r.cod, cod => getGastos(d, cod, t));
      ...
      egRows += `<tr class="${cls}">
        ...
        ${gm.map(v=>'<td class="num">'+(v?fmt(v):'—')+'</td>').join('')}
        <td class="num">${gasAcum?fmt(gasAcum):'—'}</td>
        <td class="num">${comp?fmt(comp):'—'}</td>
        <td class="num">${(gasAcum+comp)?fmt(gasAcum+comp):'—'}</td>
        <td class="num ${saldo<0?'text-danger fw-bold':''}">${fmt(saldo)}</td>
        <td class="num">${pagPer?fmt(pagPer):'—'}</td>
      ...
      const gm = meses.map(mes => getGastosMes(d, r.cod, mes));
      const gasAcum = getGastos(d, r.cod, t);
      ...
      totPD+=pd; totM[0]+=gm[0]; totM[1]+=gm[1]; totM[2]+=gm[2];
      totGasAcum+=gasAcum; totComp+=comp; totCompAcum+=compAcumPrev;
      totSaldo+=saldo; totPagPer+=pagPer; totPagAcum+=pagAcumPrev;
  ...
  const gPctEg = totPD>0 ? ((totGasAcum+totComp)/totPD*100).toFixed(1) : '0.0';
  ...
  /* ═══════════════════════════════════════════════════════
     C) KPIs
  ═══════════════════════════════════════════════════════ */
  const pctEjec = totPD>0 ? Math.round(totGasAcum/totPD*100) : 0;
  ...
  el.innerHTML = `
    ...
    <div class="row g-2 mb-3">
      ...
      <div class="col-md-3"><div class="card kpi e p-2">
        <div class="kv">${fmt(totGasAcum)}</div><div class="kl">Ejecutado Egresos (${pctEjec}%)</div>
          ...
          <div class="table-responsive">
            <table class="tg" style="font-size:10px">
              ...
              <tfoot><tr class="gtot">
                ...
                <td class="num fw-bold">${fmt(totM[0])}</td><td class="num fw-bold">${fmt(totM[1])}</td><td class="num fw-bold">${fmt(totM[2])}</td>
                <td class="num fw-bold">${fmt(totGasAcum)}</td>
                <td class="num fw-bold">${fmt(totComp)}</td>
                <td class="num fw-bold">${fmt(totGasAcum+totComp)}</td>
