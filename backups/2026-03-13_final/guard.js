...
function _validarDatos(d){
  ...
  // Limpiar gastos y compromisos huérfanos (origen='contrato' pero el contrato ya no existe)
  if(d.contratos_full){
    const idsContratos = new Set((d.contratos_full).map(c => c.id));
    if(d.contratos){
      d.contratos = d.contratos.filter(g =>
        g.origen !== 'contrato' || idsContratos.has(g.contrato_full_id)
      );
    }
    if(d.compromisos_eg){
      d.compromisos_eg = d.compromisos_eg.filter(e =>
        e.origen !== 'contrato' || idsContratos.has(e.contrato_full_id)
      );
    }
