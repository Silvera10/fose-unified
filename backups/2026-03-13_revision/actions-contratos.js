...
function _autoFechasContrato(){
  // Solo auto-llenar campos vacíos (no sobreescribir contratos existentes)
  const _si = (id, val) => { if(!$(id).value) $(id).value = val; };
  ...
  if(fechaCDP){
    ...
    // ═══ Fase Precontractual: sugerir fechas consecutivas solo si vacías ═══
    _si('mc-fecha-estudio',         _addDias(fechaCDP, 1));   // Estudio Previo
    _si('mc-fecha-ofertas',         _addDias(fechaCDP, 2));   // Invitación/Ofertas
    _si('mc-fecha-carta-propuesta', _addDias(fechaCDP, 3));   // Carta Propuesta
    _si('mc-fecha-evaluacion',      _addDias(fechaCDP, 4));   // Evaluación
    _si('mc-fecha-aceptacion',      _addDias(fechaCDP, 5));   // Aceptación Oferta

    // ═══ Fase Contractual: sugerir solo si vacías ═══
    _si('mc-fecha-inicio',          _addDias(fechaCDP, 5));   // Fecha Contrato
    _si('mc-pp-fecha-rp',           _addDias(fechaCDP, 5));   // RP
    _si('mc-sv-fecha-inicio',       $('mc-fecha-inicio').value);  // Acta Inicio = misma Fecha Inicio
  }

  // ═══ Cálculo: Fecha Fin = Base + Plazo ═══
  // Si hay Fecha Acta Inicio → el plazo corre desde ahí; si no → desde Fecha Inicio contrato
  const fechaActaInicio = $('mc-sv-fecha-inicio').value;
  const fechaInicio = $('mc-fecha-inicio').value;
  const fechaBase = fechaActaInicio || fechaInicio; // Prioridad: Acta Inicio
  const plazo = Number($('mc-plazo').value) || 0;
  const unidad = ($('mc-plazo-unidad')||{}).value || 'dias';
  if(fechaBase && plazo > 0){
    let fechaFin;
    if(unidad === 'meses'){
      const fi = new Date(fechaBase + 'T00:00:00');
      fi.setMonth(fi.getMonth() + plazo);
      fi.setDate(fi.getDate() - 1);
      fechaFin = fi.toISOString().split('T')[0];
    } else {
      fechaFin = _addDias(fechaBase, plazo);
    }
    $('mc-fecha-fin').value = fechaFin;

    // ═══ Fase de Cierre: SIEMPRE recalcular relativas a Fecha Fin ═══
    $('mc-sv-fecha-final').value = _addDias(fechaFin, 1);        // Acta Final = Fin + 1 hábil
    if($('mc-sv-fecha-liquidacion'))
      $('mc-sv-fecha-liquidacion').value = _addDias(fechaFin, 2); // Liquidación = Fin + 2 hábiles
    _si('mc-fecha-acta-recibido',   _addDias(fechaFin, 1));       // Acta Recibido (solo si vacía)
...
function abrirModalContrato(id=null){
  ...
  if(id){
    ...
    $('mc-modalidad').value = c.modalidad||''; $('mc-estado').value = c.estado||'';
...
function _migratePagos(c){
  ...
  if(c.num_egreso || c.fecha_pago || c.neto_pagar){
    c.pagos = [{
      id: uid(),
      valor: Number(c.valor) || 0,
      fecha_pago: c.fecha_pago || c.fecha_egreso || '',
...
function _autoGenerarPagos(){
  ...
  for(let i = 0; i < numPagos; i++){
    ...
    if(fechaBase && numPagos > 1){
      const fp = new Date(fechaBase + 'T00:00:00');
      if(formaPago === 'Anticipos y saldo'){
        if(i > 0){
          const fechaFin = $('mc-fecha-fin').value;
          if(fechaFin) fp.setTime(new Date(fechaFin + 'T00:00:00').getTime());
        }
      } else {
...
function _guardarPagoEditado(){
  ...
  // Verificar si todos los pagos están registrados → auto-cambiar a Liquidado
  const todosPagados = _pagosTemp.length > 0 && _pagosTemp.every(pp => pp.fecha_pago);
  if(todosPagados && $('mc-estado').value === 'En ejecucion'){
    $('mc-estado').value = 'Liquidado';
    toast('Todos los pagos registrados — Estado cambiado a Liquidado', 'success');
  } else {
...
/**
 * Sincroniza el contrato con la ejecución trimestral.
 * - Estado "En ejecución" → registra COMPROMISO en d.compromisos_eg
 * - Estado "Liquidado"    → registra GASTO en d.contratos (con fecha egreso)
 * Limpia el registro anterior si cambia de estado.
 */
function _syncContratoTrimestral(contrato, d){
  ...
  // ═══ 1. LIMPIAR registros anteriores de este contrato ═══
  ...
  if(!rubro || valor <= 0) return;

  const pagos = contrato.pagos || [];
  const pagosConFecha = pagos.filter(p => p.fecha_pago && Number(p.valor) > 0);
  const totalPagado = pagosConFecha.reduce((s, p) => s + (Number(p.valor) || 0), 0);
  ...
  if(estado === 'En ejecucion'){
    // Registrar gastos de pagos ya realizados (con fecha)
    pagosConFecha.forEach(p => _pushGasto(p));

    // Compromiso = saldo pendiente por pagar
    const saldoPendiente = valor - totalPagado;
    if(saldoPendiente > 0){
      const fechaRef = contrato.fecha_cdp || contrato.fecha_inicio || '';
      const tm = _trimDeFecha(fechaRef);
      if(tm){
        const compId = uid();
  ...
  if(estado === 'Liquidado'){
    if(pagosConFecha.length > 0){
      // Múltiples gastos — uno por cada pago con fecha
      pagosConFecha.forEach(p => _pushGasto(p));
    } else {
      // Fallback legacy: un solo gasto con fecha egreso del contrato
      const fechaEgreso = contrato.fecha_egreso;
      if(!fechaEgreso) return;
      const tm = _trimDeFecha(fechaEgreso);
      if(!tm) return;
      const gastoId = uid();
      d.contratos.push({
        id:             gastoId,
        ...
        neto_pagar:          Number(contrato.neto_pagar) || 0,
        reteica:             Number(contrato.reteica) || 0
      });
...
function guardarContrato(){
  ...
  // ═══ Alerta RP pendiente (no bloquea, solo avisa) ═══
  const _rpVal = ($('mc-pp-rp')?.value || '').trim();
  if(numero && !_rpVal){
    if(!confirm('⚠️ El N° de RP está vacío.\n\n¿Desea guardar el contrato sin RP?\n\n• Aceptar = Guardar sin RP\n• Cancelar = Volver a digitar el RP')){
  ...
  if(_fuente && valor > 0){
    const d_tmp = DB.load();
    let saldoFuente = 0;
    for(let t=1;t<=4;t++) saldoFuente += (typeof getPresupDispIng==='function' ? getPresupDispIng(d_tmp,_fuente,t) : 0);
  ...
  // Si datos incompletos, forzar estado borrador
  let estadoFinal = incompleto ? 'En ejecucion' : $('mc-estado').value;

  // Auto-liquidar si todos los pagos están registrados
  if(_pagosTemp.length > 0 && _pagosTemp.every(pp => pp.fecha_pago) && estadoFinal === 'En ejecucion'){
...
function eliminarContrato(id){
  ...
  const d = DB.load();
  // ═══ Eliminar gasto Y compromiso trimestral vinculados ═══
