...
function _sifseCalcGastos(d, tMax){
  ...
  rubros.forEach(r => {
    const m = map[r.cod];
    if(!m || !m.fuente || !m.gasto) return;
    const k = m.fuente + '|' + m.gasto;
    if(!agg[k]) agg[k] = {fuente:m.fuente, gasto:m.gasto, ini:0, def:0, comp:0, oblig:0, pagos:0};
    agg[k].ini   += Number(r.ini||0);
    agg[k].def   += _sifseDefEg(d, r.cod, tMax);
    agg[k].comp  += _sifseCompAcum(d, r.cod, tMax);
    agg[k].oblig += _sifseObligAcum(d, r.cod, tMax);
    agg[k].pagos += _sifsePagosAcum(d, r.cod, tMax);
  });
