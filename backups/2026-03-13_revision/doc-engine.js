...
/* ── Obtener plantilla (desde DOC_TEMPLATES embebido o fetch fallback) ── */
async function fetchTemplate(name){
  if(_tplCache[name]) return _tplCache[name];
  // Buscar en plantillas embebidas (doc-templates.js)
  ...
  // Fallback: fetch (funciona con servidor local)
  try {
    ...
    throw new Error('No se pudo cargar la plantilla: '+key+' — '+e.message);
  }
}
...
function renderDocPanel(contratoId){
  ...
  function pagosBtns(doc, cId){
    if(pagos.length <= 1) return docBtn(doc, cId); // pago único: botón normal
