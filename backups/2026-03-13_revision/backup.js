...
function renderListaInstituciones(){
  ...
  const filas = list.map(i => {
    ...
    return `<tr style="${esActiva?'background:#f1f8e9;font-weight:600':''}">
      <td class="ps-2">${i.nombre}${esActiva?' <span class="badge bg-success" style="font-size:9px">ACTIVA</span>':''}</td>
      <td class="text-center">${vig||'—'}</td>
      <td class="text-end pe-2" style="white-space:nowrap">
        ${!esActiva?`<button class="btn btn-success btn-sm py-0 px-2" style="font-size:10px" onclick="cambiarInstitucion('${i.id}')"><i class="bi bi-box-arrow-in-right me-1"></i>Activar</button>`:''}
        <button class="btn btn-outline-warning btn-sm py-0 px-1" style="font-size:10px" onclick="abrirModalInstForm('${i.id}')"><i class="bi bi-pencil"></i></button>
        ${list.length>1?`<button class="btn btn-outline-danger btn-sm py-0 px-1" style="font-size:10px" onclick="eliminarInstitucion('${i.id}')"><i class="bi bi-trash"></i></button>`:''}
      </td>
    </tr>`;
  }).join('');
  el.innerHTML = `<div class="col-12"><table class="table table-sm table-bordered table-hover mb-0" style="font-size:11px">
    <thead style="background:#1a3a5c;color:#fff"><tr>
      <th class="ps-2">Institución</th><th class="text-center" style="width:80px">Vigencia</th><th class="text-end pe-2" style="width:160px">Acciones</th>
