...
function _excelEgresos(wb, d, trim){
  ...
  grupos.forEach(g => {
    ...
    combinada.forEach(rb => {
      const isGrp = rb._isVirtual || rb.esGrupo === true;
      const n = Math.min(getNivel(rb.cod), 5);
      const row = ws.getRow(r);

      if(isGrp){
        const hijos = d.rubros.filter(x => x.tipo===g.tipo && !x.esGrupo && x.cod.startsWith(rb.cod+'.'));
        let pSI=0,pAdi=0,pRed=0,pCre=0,pCco=0,pPD=0,pMes=new Array(nMes).fill(0),pGas=0,pComp=0,pSaldo=0,pPagPer=0,pPagAcum=0;
        hijos.forEach(x => {
          let si2,adi2,red2,cre2,cco2,pd2,mesV2;
          if(esTrim){
            ...
            mesV2=mesesCols.map(mn=>getGastosMes(d,x.cod,mn));
          } else {
          ...
          const gas2=mesV2.reduce((a,b)=>a+b,0);
          const comp2=getCompromisoEgTrim(d,x.cod,esTrim?trim:0);
          const pagPer2=gas2;
          const pagAcum2=esTrim?[1,2,3,4].filter(t2=>t2<trim).reduce((s,t2)=>s+getGastos(d,x.cod,t2),0):0;
          pSI+=si2;pAdi+=adi2;pRed+=red2;pCre+=cre2;pCco+=cco2;pPD+=pd2;
          mesV2.forEach((v,i)=>pMes[i]+=v);pGas+=gas2;pComp+=comp2;pSaldo+=(pd2-gas2-comp2);
          pPagPer+=pagPer2;pPagAcum+=pagAcum2;
        });
        ...
        const vals = [pSI,pAdi,pRed,pCre,pCco,pPD,...pMes,pGas,pComp,pGas,pSaldo,pPagPer,pPagAcum,0];
        vals.forEach((v,i) => _xCell(row,i+3,v,{fill,font,numFmt:_XS.numFmt}));
        _xCell(row,nCols,pPD>0?((pGas+pComp)/pPD*100):0,{fill,font,numFmt:_XS.pctFmt});
      } else {
        ...
        const gas=mesV.reduce((a,b)=>a+b,0);
        const comp=getCompromisoEgTrim(d,rb.cod,esTrim?trim:0);
        const pagPer=gas;
        const pagAcum=esTrim?[1,2,3,4].filter(t2=>t2<trim).reduce((s,t2)=>s+getGastos(d,rb.cod,t2),0):0;
        const saldo=pd-gas-comp;
        gSI+=si;gAdi+=adi;gRed+=red;gCre+=cre;gCco+=cco;gPD+=pd;
        mesV.forEach((v,i)=>gMes[i]+=v);gGas+=gas;gComp+=comp;gSaldo+=saldo;gPagPer+=pagPer;gPagAcum+=pagAcum;

        _xCell(row,1,rb.cod,{font:_XS.fontNorm});
        _xCell(row,2,rb.con,{font:_XS.fontNorm});
        const vals = [si,adi,red,cre,cco,pd,...mesV,gas,comp,gas,saldo,pagPer,pagAcum,0];
        vals.forEach((v,i) => _xCell(row,i+3,v,{numFmt:_XS.numFmt,font:_XS.fontNorm}));
        _xCell(row,nCols,pd>0?((gas+comp)/pd*100):0,{numFmt:_XS.pctFmt,font:_XS.fontNorm});
      }
      r++;
    });
  });
  ...
  const totVals = [gSI,gAdi,gRed,gCre,gCco,gPD,...gMes,gGas,gComp,gGas,gSaldo,gPagPer,gPagAcum,0];
  totVals.forEach((v,i) => _xCell(totRow,i+3,v,{fill:_XS.totFill,font:_XS.fontWhite,numFmt:_XS.numFmt}));
  _xCell(totRow,nCols,gPD>0?((gGas+gComp)/gPD*100):0,{fill:_XS.totFill,font:_XS.fontWhite,numFmt:_XS.pctFmt});
}
...
function _excelIngresos(wb, d, trim){
  ...
  _xCell(totRow,nCols,gPD>0?(gRec/gPD*100):0,{fill:_XS.totFill,font:_XS.fontWhite,numFmt:_XS.pctFmt});
}
...
function _excelRelacionGastos(wb, d, trim){
  ...
  ['#','Trim.','Fecha','Tipo Doc.','N° Doc.','Fecha CDP','N° CDP','Fecha RP','N° RP','Comp.',
   'Concepto','Proveedor','Rubro','Fecha Contrato','N° Contrato','Valor','Pub.','Cont.'].forEach((h,i)=>
  ...
  ws.columns=[{width:4},{width:10},{width:11},{width:8},{width:12},{width:11},{width:10},{width:11},{width:10},{width:8},
    {width:35},{width:25},{width:14},{width:11},{width:12},{width:15},{width:5},{width:5}];
  ...
  cs.forEach((c,i)=>{
    ...
    const vals=[i+1,tl[c.trim]||'T'+c.trim,c.fecha||'',c.tipodoc||'',c.numdoc||'',
      ...
      c.concepto||'',c.prov||'',c.cod_rubro||'',c.fecha_contrato||'',c.ncon||'',
      Number(c.valor)||0,c.pub?'SÍ':'NO',c.cont?'SÍ':'NO'];
    vals.forEach((v,j)=>{
      const opts={font:_XS.fontNorm};
      if(j===15) opts.numFmt=_XS.numFmt;
      _xCell(row,j+1,v,opts);
    });
    r++;
  });
  const totRow=ws.getRow(r);
  for(let c=1;c<=15;c++)_xCell(totRow,c,c===1?'TOTAL':'',{fill:_XS.totFill,font:_XS.fontWhite});
  _xCell(totRow,16,tot,{fill:_XS.totFill,font:_XS.fontWhite,numFmt:_XS.numFmt});
  _xCell(totRow,17,'',{fill:_XS.totFill,font:_XS.fontWhite});
...
function _excelCierre(wb, d){
  ...
  let r1=6,gIngSI=0,gIngFin=0;
  (d.rubros_ing||[]).filter(x=>!x.esGrupo).forEach(rb=>{
    const si=Number(rb.ini)||0;
    let adi=0,red=0,cre=0,cco=0;
    [1,2,3,4].forEach(t=>{const mi=getModsIng(d,rb.cod,t);adi+=Number(mi.adi);red+=Number(mi.red);cre+=Number(mi.cre);cco+=Number(mi.cco);});
    const fin=si+adi-red+cre-cco;
    gIngSI+=si;gIngFin+=fin;
    const row=ws1.getRow(r1);
    _xCell(row,1,rb.cod,{font:_XS.fontNorm});_xCell(row,2,rb.guia||'',{font:_XS.fontNorm});
    _xCell(row,3,rb.con,{font:_XS.fontNorm});
    [si,adi,red,cre,cco,fin,fin].forEach((v,i)=>_xCell(row,4+i,v,{numFmt:_XS.numFmt,font:_XS.fontNorm}));
    r1++;
  });
  const totI1=ws1.getRow(r1);
  for(let c=1;c<=3;c++)_xCell(totI1,c,c===3?'TOTAL INGRESOS':'',{fill:_XS.totFill,font:_XS.fontWhite});
  for(let c=4;c<=10;c++)_xCell(totI1,c,c===9?gIngFin:c===10?gIngFin:c===4?gIngSI:0,{fill:_XS.totFill,font:_XS.fontWhite,numFmt:_XS.numFmt});
