...
function genEgresos(d, trim){
  ...
  grupos.forEach(g => {
    ...
    combinada.forEach(r => {
      const isGrp = r._isVirtual || r.esGrupo === true;
      ...
      if (isGrp){
        ...
        const pP = pPD>0 ? ((pGas+pComp)/pPD*100).toFixed(1) : '0.0';
        rows += `<tr style="background:${cfg2.bg};color:${cfg2.clr};font-weight:${cfg2.fw};font-size:10px">
          ...
          ${pMes.map(v=>'<td class="num">'+(v?fmt(v):'—')+'</td>').join('')}
          <td class="num">${pGas>0?fmt(pGas):'—'}</td>
          <td class="num">${pComp>0?fmt(pComp):'—'}</td>
          <td class="num">${pGas>0?fmt(pGas):'—'}</td>
          <td class="num${pSaldo<0?' saldo-neg':''}">${fmt(pSaldo)}</td>
          <td class="num">${pGas>0?fmt(pGas):'—'}</td><td class="num">—</td><td class="num">—</td>
        ...
        const comp=getCompromisoEgTrim(d,r.cod,esTrim?trim:0);
        const pagPer=gas;
        const pagAcum=esTrim?[1,2,3,4].filter(t2=>t2<trim).reduce((s,t2)=>s+getGastos(d,r.cod,t2),0):0;
        const saldo=pd-gas-comp;
        const p=pd>0?((gas+comp)/pd*100).toFixed(1):'0.0';

        sSI+=si;sAdi+=adi;sRed+=red;sCre+=cre;sCco+=cco;sPD+=pd;
        mesV.forEach((v,i)=>sMes[i]+=v);sGas+=gas;sComp+=comp;sSaldo+=saldo;sPagPer+=pagPer;sPagAcum+=pagAcum;
        ...
        rows += `<tr>
          ...
          <td class="num">${pagPer>0?fmt(pagPer):'—'}</td>
          <td class="num">${pagAcum>0?fmt(pagAcum):'—'}</td>
          <td class="num">—</td>
  ...
  const gP = gPD>0 ? ((gGas+gComp)/gPD*100).toFixed(1) : '0.0';
  rows += `<tr class="gtot"><td colspan="2">TOTALES</td>
    ...
    <td class="num">${fmt(gGas)}</td><td class="num">${fmt(gComp)}</td><td class="num">${fmt(gGas)}</td>
    <td class="num${gSaldo<0?' saldo-neg':''}">${fmt(gSaldo)}</td>
    <td class="num">${fmt(gPagPer)}</td><td class="num">${fmt(gPagAcum)}</td><td class="num">—</td>
...
function genIngresos(d, trim){
  ...
  const gP = gPD>0?(gRec/gPD*100).toFixed(1):'0.0';
  rows += `<tr class="gtot"><td colspan="2">TOTALES</td>
    <td class="num">${fmt(gSI)}</td><td class="num">${fmt(gAdi)}</td><td class="num">${fmt(gRed)}</td>
...
function genBalancePresupuestal(d, trim){
  ...
  const pctIngRec = ingDef>0?(ingRec/ingDef*100).toFixed(1):'0.0';
